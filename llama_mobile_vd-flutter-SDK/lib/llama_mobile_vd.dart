...
/// A builder for creating MMapVectorStore files
class MMapVectorStoreBuilder {
  final int _builderId;
  ...
  static Future<MMapVectorStoreBuilder> create({
    ...
    final builderId =
        await _channel.invokeMethod<int>('mmapVectorStoreBuilderCreate', {
      'dimension': dimension,
      'metric': metric.value,
    });

    if (builderId == null) {
      throw Exception('Failed to create MMapVectorStoreBuilder');
    }

    return MMapVectorStoreBuilder._(builderId);
  }

  MMapVectorStoreBuilder._(this._builderId);
  ...
  /// - Throws: An error if the vector could not be added
  Future<void> addVector(List<double> vector, int id) async {
    await _channel.invokeMethod('mmapVectorStoreBuilderAddVector', {
  ...
  ///   - capacity: The number of vectors to reserve space for
  /// - Throws: An error if the operation fails
  Future<void> reserveCapacity(int capacity) async {
    await _channel.invokeMethod('mmapVectorStoreBuilderReserveCapacity', {
      'builderId': _builderId,
      'capacity': capacity,
    });
  }
  ...
  /// - Parameters:
  ///   - path: The path where the store should be saved
  /// - Throws: An error if the store could not be saved
  Future<void> save(String path) async {
    await _channel.invokeMethod('mmapVectorStoreBuilderSave', {
      'builderId': _builderId,
      'path': path,
    });
  }
  ...
  Future<int> get size async {
    return await _channel.invokeMethod<int>('mmapVectorStoreBuilderSize', {
          'builderId': _builderId,
        }) ??
        0;
  }
  ...
  Future<int> get dimension async {
    return await _channel.invokeMethod<int>('mmapVectorStoreBuilderDimension', {
          'builderId': _builderId,
        }) ??
        0;
  }
  ...
  Future<void> dispose() async {
    await _channel.invokeMethod('mmapVectorStoreBuilderDestroy', {
      'builderId': _builderId,
    });
  }
}
...
class MMapVectorStore {
  ...
  Future<List<SearchResult>> search(List<double> queryVector, int k) async {
    final results =
        await _channel.invokeMethod<List<dynamic>>('mmapVectorStoreSearch', {
      'storeId': _storeId,
      'queryVector': Float32List.fromList(queryVector),
      'k': k,
    });
  ...
  Future<DistanceMetric> get metric async {
    final metricValue =
        await _channel.invokeMethod<int>('mmapVectorStoreMetric', {
      'storeId': _storeId,
    });
