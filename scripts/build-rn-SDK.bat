...
REM Build script for LlamaMobileVD React Native SDK on Windows
...
for /f "usebackq tokens=*" %%a in (`get_config_value react-native RN_ANDROID_VARIANT "release"`) do set "ANDROID_VARIANT=%%a"
for /f "usebackq tokens=*" %%a in (`get_config_value react-native RN_IOS_CONFIGURATION "Release"`) do set "IOS_CONFIGURATION=%%a"
...
set "RN_SDK_DIR=%PROJECT_ROOT%\llama_mobile_vd-react-native-SDK"
set "ios_FRAMEWORK_DEST=%RN_SDK_DIR%\ios\LlamaMobileVD.framework"
set "ANDROID_JNI_DEST=%RN_SDK_DIR%\android\src\main\jniLibs"
set "ANDROID_JAVA_SRC_DEST=%RN_SDK_DIR%\android\src\main\java\com\llamamobile\vd"
...
echo === Building Android Components for React Native SDK ===
...
REM Copy Android files to React Native SDK
echo Copying Android files to React Native SDK...
...
echo Warning: iOS framework for React Native can only be built on macOS

REM Final steps
echo === React Native SDK Build Complete ===
echo Android components built and copied successfully

echo To complete React Native SDK setup:
1. For Android: All required components are built
2. For iOS: Build the iOS framework on macOS using build-ios.sh
3. Run: npm install in your React Native project
4. Run: cd ios && pod install && cd ..
5. Run: react-native run-android or react-native run-ios (on macOS)

echo React Native SDK directory: %RN_SDK_DIR%
exit /b 0