...
/**
 * A builder for creating MMapVectorStore instances
 */
public class MMapVectorStoreBuilder implements AutoCloseable {
    ...
    public MMapVectorStoreBuilder(int dimension, DistanceMetric metric) {
        ...
        if (pointer == 0L) {
            throw new IllegalStateException("Failed to create MMapVectorStore builder");
        }
    }
    ...
    /**
     * Reserve capacity for vectors in the builder
     *
     * @param capacity The number of vectors to reserve capacity for
     * @throws IllegalArgumentException If the capacity is invalid
     */
    public void reserve(int capacity) {
        if (!reserve(pointer, capacity)) {
            throw new IllegalArgumentException("Failed to reserve capacity");
        }
    }

    /**
     * Save the MMapVectorStore to a file
     *
     * @param filename The name of the file to save the vector store to
     * @throws IllegalArgumentException If the file path is invalid
     */
    public void save(String filename) {
        if (!save(pointer, filename)) {
            throw new IllegalArgumentException("Failed to save MMapVectorStore");
        }
    }

    /**
     * Get the number of vectors added to the builder
     *
     ...
     *
     * @return The dimension of vectors in the builder
     */
    ...
    private native int dimension(long builder);
}

/**
 * A memory-mapped vector store for efficiently storing and searching large vector datasets
 */
public class MMapVectorStore implements AutoCloseable {
     ...
     *
     * @param filename The name of the file containing the vector store
     * @return A new MMapVectorStore instance
     * @throws IllegalStateException If the vector store could not be opened
     */
    public static MMapVectorStore open(String filename) {
        long storePointer = open(filename);
        if (storePointer == 0L) {
            throw new IllegalStateException("Failed to open MMapVectorStore from file: " + filename);
        }
     ...
     * @return The vector with the specified ID, or null if not found
     */
    public float[] get(int id) {
        int dim = dimension();
        float[] vector = new float[dim];
        if (get(pointer, id, vector)) {
            return vector;
        }
        return null;
    }
    ...
    public SearchResult[] search(float[] queryVector, int k) {
        ...
        try {
            SearchResult[] results = new SearchResult[resultCount[0]];
            for (int i = 0; i < resultCount[0]; i++) {
                int vectorId = getResultId(resultsPtr, i);
                float distance = getResultDistance(resultsPtr, i);
                results[i] = new SearchResult(vectorId, distance);
            }
            return results;
        } finally {
            freeSearchResults(resultsPtr);
    ...
    /**
     * Check if the vector store contains a vector with the specified ID
     *
     * @param id The ID to check
     * @return True if the vector store contains the ID, false otherwise
     */
    public boolean contains(int id) {
        int[] contains = new int[1];
        if (!contains(pointer, id, contains)) {
            return false;
        }
        return contains[0] != 0;
    }
     ...
     *
     * @return The dimension of vectors in the store
     */
    public int getDimension() {
        return dimension(pointer);
    }
     ...
     *
     * @return The distance metric used by the store
     */
    public DistanceMetric getMetric() {
        int metricValue = metric(pointer);
        for (DistanceMetric metric : DistanceMetric.values()) {
            if (metric.getValue() == metricValue) {
                return metric;
            }
        }
        throw new IllegalStateException("Invalid distance metric: " + metricValue);
    }
    ...
    // JNI methods
    private static native long open(String filename);
    private native boolean get(long store, int id, float[] vector);
    private native long search(long store, float[] queryVector, int vectorSize, int k, int[] resultCount);
    private native boolean contains(long store, int id, int[] contains);
    private native int getCount(long store);
    private native int dimension(long store);
    private native int metric(long store);
    private native void close(long store);
    private native void freeSearchResults(long results);
    private native int getResultId(long results, int index);
    private native float getResultDistance(long results, int index);
}
