...
void main() {
    ...
    channel.setMockMethodCallHandler((MethodCall methodCall) async {
      ...
      switch (methodCall.method) {
        ...
        case 'mmapVectorStoreBuilderCreate':
          return 1; // Return mock builder ID
  ...
  group('MMapVectorStoreBuilder', () {
    test('should create an MMapVectorStoreBuilder', () async {
    ...
    test('should add vectors to the builder', () async {
      final builder = await MMapVectorStoreBuilder.create(
        dimension: 512,
        metric: DistanceMetric.l2,
      );
      ...
      final vector = List<double>.generate(512, (_) => 0.5);
      expect(() => builder.addVector(vector, 1), returnsNormally);
    });
    ...
    test('should reserve capacity', () async {
      ...
      expect(() => builder.reserveCapacity(1000), returnsNormally);
    });

    test('should save the store to a file', () async {
      final builder = await MMapVectorStoreBuilder.create(
        dimension: 512,
        metric: DistanceMetric.l2,
      );

      expect(() => builder.save('/path/to/vectorstore.mmap'), returnsNormally);
    });

    test('should get correct size', () async {
      final builder = await MMapVectorStoreBuilder.create(
        dimension: 512,
        metric: DistanceMetric.l2,
      );

      final size = await builder.size;
      expect(size, 5); // Matches mock response
    });
    ...
    test('should get correct dimension', () async {
      ...
      final dimension = await builder.dimension;
      expect(dimension, 512); // Matches mock response
    });

    test('should dispose of the builder', () async {
      final builder = await MMapVectorStoreBuilder.create(
        dimension: 512,
        metric: DistanceMetric.l2,
      );
    ...
    test('should support different distance metrics', () async {
      // Test L2
      final builderL2 = await MMapVectorStoreBuilder.create(
        dimension: 128,
        metric: DistanceMetric.l2,
      );
      expect(builderL2, isNotNull);

      // Test Cosine
      final builderCosine = await MMapVectorStoreBuilder.create(
        dimension: 128,
        metric: DistanceMetric.cosine,
      );
      expect(builderCosine, isNotNull);

      // Test Dot
      final builderDot = await MMapVectorStoreBuilder.create(
        dimension: 128,
        metric: DistanceMetric.dot,
      );
      expect(builderDot, isNotNull);
    });
