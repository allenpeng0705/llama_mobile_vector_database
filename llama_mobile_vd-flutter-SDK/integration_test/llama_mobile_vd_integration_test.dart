...
void main() {
  ...
  group('LlamaMobileVDFlutterSdk Integration Tests', () {
    ...
    testWidgets('VectorStore create and basic operations', (WidgetTester tester) async {
      ...
      final results = await vectorStore.search(vector, k: 1);
    ...
    testWidgets('HNSWIndex create and basic operations', (WidgetTester tester) async {
      // Create an HNSWIndex
      final hnswIndex = await HNSWIndex.create(64, DistanceMetric.cosine, maxElements: 100);
      ...
      // Search for the vector
      final results = await hnswIndex.search(vector, k: 1);
      expect(results, isNotEmpty);
      expect(results.first.id, equals(id));
    ...
    testWidgets('MMapVectorStore create and basic operations', (WidgetTester tester) async {
      // Create a temporary file path
      final tempPath = 'test_mmap_vector_store.bin';

      // Create and save MMapVectorStore
      final builder = await MMapVectorStoreBuilder.create(64, DistanceMetric.dot);
      expect(builder, isNotNull);

      // Add vectors
      for (int i = 0; i < 5; i++) {
        final vector = List.generate(64, (index) => (i * 100 + index).toDouble());
      ...
      // Search for a vector
      final queryVector = List.generate(64, (index) => (2 * 100 + index).toDouble());
      final results = await mmapVectorStore.search(queryVector, k: 1);
      expect(results, isNotEmpty);
